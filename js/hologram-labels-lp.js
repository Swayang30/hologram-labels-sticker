/* ==========================================================================
   Holoflex — Hologram Labels landing page + thank-you page
   /js/hologram-labels-lp.js

   Vanilla JS, no dependencies. Loaded with `defer` on both
   /hologram_labels/ (index.html) and thank-you-lp.html. Sections that need elements
...
(function () {
  ...
  var ENDPOINT = 'submit-enquiry.php';       // relative: the page lives in a sub-folder (/hologram_labels/)
    ...
    fetch(ENDPOINT, {
      ...
      .then(function (data) {
        if (data && data.success) {
          redirecting = true;                 // keep the button disabled while the page changes
          redirectToThankYou(location, userData);
        } else {
          showFormError(form, (data && data.error) || 'Sorry, something went wrong. Please try again, or WhatsApp us on +91 62923 00439.');
        }
      })
      .catch(function () {
        showFormError(form, 'We could not send your enquiry just now. Please try again, or call or WhatsApp +91 62923 00439.');
      })
      .then(function () {
